...
$searchTerm = '';
$conditions = [];
$conditions[] = "educational_attainment = 'CARER PROGRAM'";

if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
  $searchTerm = trim($_GET['search']);
  $sq = mysqli_real_escape_string($conn, $searchTerm);
  $conditions[] = "(lrn_no LIKE '%$sq%'
                    OR CONCAT(first_name, ' ', middle_name, ' ', last_name) LIKE '%$sq%'
                    OR contact_no LIKE '%$sq%'
                    OR address LIKE '%$sq%'
                    OR barangay LIKE '%$sq%'
                    OR disability_type LIKE '%$sq%'
                    OR specific_disability LIKE '%$sq%')";
}
...
if (!empty($programFilter) && in_array($programFilter, $validPrograms)) {
  $ep = mysqli_real_escape_string($conn, $programFilter);
  $conditions[] = "previous_level = '$ep'";
}

$whereClause = 'WHERE ' . implode(' AND ', $conditions);

$countRow = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM enrollees $whereClause"));
...
$all_enrollees = [];
$result = mysqli_query($conn, "SELECT * FROM enrollees $whereClause ORDER BY enrollee_id DESC LIMIT $itemsPerPage OFFSET $offset");
while ($row = mysqli_fetch_assoc($result)) $all_enrollees[] = $row;
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="sb-nav-fixed">
  ...
  <div id="layoutSidenav">
    ...
    <div id="layoutSidenav_content">
      <main>
        <section class="admin-panel" aria-label="Intervention panel">
          <div class="d-flex align-items-center mb-3">
            <h2 class="m-0">Intervention</h2>
          </div>


          <form method="GET" class="mb-3">
            <?php if (!empty($programFilter)): ?>
            ...
            <div class="input-group">
              ...
              <?php if (!empty($searchTerm) || !empty($programFilter)): ?>
                <a href="intervention.php" class="btn btn-outline-secondary">
          ...
          <div class="grade-nav-wrapper mb-3">
            <ul class="nav grade-nav flex-nowrap">
              ...
              foreach ($validPrograms as $prog):
                $isActive = ($programFilter === $prog);
                $href = '?program=' . urlencode($prog) . (!empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '');
                $clearHref = 'intervention.php' . (!empty($searchTerm) ? '?search=' . urlencode($searchTerm) : '');
              ?>
                <li class="nav-item">
                ...
                <?php else: ?>
                  <tr>
                    <td colspan="7" class="text-center py-4">
                      ...
                      <p class="text-muted mt-2">No enrollees found<?= (!empty($searchTerm) || !empty($programFilter)) ? ' matching your filter.' : '.' ?></p>
                    </td>
                  </tr>
                <?php endif; ?>
              </tbody>
            ...
            <nav aria-label="Page navigation" class="mt-4">
              <ul class="pagination justify-content-center">
                <li class="page-item <?= $currentPage <= 1 ? 'disabled' : '' ?>">
                  <a class="page-link" href="?page=<?= $currentPage - 1 ?><?= !empty($programFilter) ? '&program=' . urlencode($programFilter) : '' ?><?= !empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '' ?>">Previous</a>
                </li>

                <?php
                $startPage = max(1, $currentPage - 2);
                $endPage = min($totalPages, $currentPage + 2);
                $pageExtra = (!empty($programFilter) ? '&program=' . urlencode($programFilter) : '') . (!empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '');
